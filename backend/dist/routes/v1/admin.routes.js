...
router.post('/role-request', authenticate, asyncHandler(async (req, res) => {
...
router.get('/role-requests', authenticate, requireRole('admin'), asyncHandler(async (req, res) => {
...
router.patch('/role-requests/:id', authenticate, requireRole('admin'), asyncHandler(async (req, res) => {
    ...
    if (userRow) {
        const { subject, html } = buildRoleEmail(userRow.name, action);
