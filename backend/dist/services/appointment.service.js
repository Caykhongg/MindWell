...
export class AppointmentService {
    ...
    async getTherapistEntries(therapistId, page, limit) {
    ...
    async createEntry(userId, data) {
        ...
        const availableTherapists = await db
            ...
            .where(and(eq(users.role, 'therapist'), eq(users.isActive, true), eq(counselorAvailability.dayOfWeek, dayOfWeek), eq(counselorAvailability.isAvailable, true), sql `${counselorAvailability.startTime} <= ${timeStr}::time`, sql `${counselorAvailability.endTime} >= ${timeStr}::time`));
    ...
    async updateStatus(id, userId, userRole, data) {
        const appointment = await this.appointmentRepo.findById(id);
        if (!appointment)
            throw new NotFoundError('lịch hẹn');
    ...
    async cancelEntry(id, userId, userRole) {
        const appointment = await this.appointmentRepo.findById(id);
        if (!appointment)
            throw new NotFoundError('lịch hẹn');
        const isOwner = appointment.userId === userId;
